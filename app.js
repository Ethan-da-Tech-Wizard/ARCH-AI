...
extendSection("Configure System", {
  ...
  commands: [
    ...
    },
    {
      ...
      expected: "Usually no output.",
      fails: "If /boot is not writable, confirm the EFI partition is mounted at /boot."
    },
    {
    ...
    },
    {
      ...
      expected: "Output indicates systemd-boot was installed and EFI boot entries were created or updated.",
      fails: "If bootctl cannot find an EFI System Partition, confirm UEFI mode and that the ESP is mounted at /boot."
    },
    {
...
function commandSafetyType(command) {
  ...
  if (/^(ls|lsblk|fdisk\s+-l|blkid|findmnt|grep|ping|resolvectl|whoami|man|localectl|ip\s|ps\b|pstree\b|journalctl\b|pacman\s+-Q|cat\b|systemctl\s+show)\b/.test(text)) {
